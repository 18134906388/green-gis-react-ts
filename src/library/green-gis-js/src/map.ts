...
export class Map extends Subject {
  ...
  constructor(id: string | HTMLDivElement, option?: any) {
    ...
    //option
    this._option.disableDoubleClick =
      option && option.hasOwnProperty('disableDoubleClick') ? option.disableDoubleClick : false;

    this._container =
      id instanceof HTMLDivElement ? id : (document.getElementById(id) as HTMLDivElement);
    //create canvas
    ...
    this._viewer.on('mouseover', () => {
      Utility.addClass(this._canvas, 'green-hover');
    });
    this._viewer.on('mouseout', () => {
      Utility.removeClass(this._canvas, 'green-hover');
    });

    //editor
    this._editor = new Editor(this);
    this._editor.on('mouseover', () => {
      Utility.addClass(this._canvas, 'green-hover');
    });
    this._editor.on('mouseout', () => {
      Utility.removeClass(this._canvas, 'green-hover');
    });
    this._editor.on('startedit', () => {
      this._viewer.redraw();
    });
    this._editor.on('stopedit', () => {
      this._viewer.redraw();
    });
   ...
   */
  setView(center: number[] = [0, 0], zoom = 3) {
    this._center = center;
   ...
   */
  insertLayer(layer: Layer, index = -1) {
  ...
  updateExtent() {
    ...
    const x1 = (0 - matrix.e) / matrix.a,
      y1 = (0 - matrix.f) / matrix.d,
      x2 = (this._canvas.width - matrix.e) / matrix.a,
      y2 = (this._canvas.height - matrix.f) / matrix.d;
    this._extent = new Bound(
      Math.min(x1, x2),
      Math.min(y1, y2),
      Math.max(x1, x2),
      Math.max(y1, y2),
  ...
  _onTouchStart(event) {
    if (event.touches.length == 2) {
      ...
      const diffX = event.touches[0].clientX - event.touches[1].clientX;
      const diffY = event.touches[0].clientY - event.touches[1].clientY;
  ...
  _onTouchMove(event) {
    ...
    if (event.touches.length == 2 && this._touch.zooming) {
      // If pinch-zooming
      const diffX = event.touches[0].clientX - event.touches[1].clientX;
      const diffY = event.touches[0].clientY - event.touches[1].clientY;
      const new_finger_dist = Math.sqrt(diffX * diffX + diffY * diffY); // Get current distance between fingers
      ...
      const a1 = matrix.a,
        e1 = matrix.e,
        x1 = (event.touches[0].clientX + event.touches[1].clientX) / 2,
        x2 = x1; //放大到中心点 x2 = this._canvas.width / 2
      const e = (x2 - scale * (x1 - e1) - e1) / a1;
      const d1 = matrix.d,
        f1 = matrix.f,
        y1 = (event.touches[0].clientY + event.touches[1].clientY) / 2,
        y2 = y1; //放大到中心点 y2 = this._canvas.height / 2
      const f = (y2 - scale * (y1 - f1) - f1) / d1;
      this._ctx.transform(scale, 0, 0, scale, e, f);
      this.redraw();
