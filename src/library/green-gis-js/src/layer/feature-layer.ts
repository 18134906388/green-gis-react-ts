...
export class FeatureLayer extends Layer {
  ...
  draw(
    ...
    projection: Projection = new WebMercator(),
    extent: Bound = projection.bound,
    zoom = 10,
  ) {
    if (this.visible && this._zoom[0] <= zoom && this._zoom[1] >= zoom) {
      //过滤可见视图范围内的要素
      const features = this._featureClass.features.filter((feature: Feature) =>
        feature.intersect(projection, extent),
      );
      ...
      const _getSymbol = feature => {
        if (this._renderer instanceof SimpleRenderer) {
          ...
          const item = renderer.items.find(
            item => item.value == feature.properties[renderer.field.name],
          );
          ...
          const item = renderer.items.find(
            item =>
              item.low <= feature.properties[renderer.field.name] &&
              item.high >= feature.properties[renderer.field.name],
          );
          if (item) return item.symbol;
        }
      };
  ...
  contain(
    ...
    if (this.visible && this._zoom[0] <= zoom && this._zoom[1] >= zoom) {
      ...
      const features = this._featureClass.features
        .filter((feature: Feature) => feature.intersect(projection, extent))
        .filter((feature: Feature) => {
          return feature.contain(screenX, screenY, event);
        });
      if (features.length > 0) {
        if (event == 'dblclick') {
          features[0].emit('dblclick', {
            feature: features[0],
            screenX: screenX,
            screenY: screenY,
          });
        } else if (event == 'click') {
          features[0].emit('click', {
            feature: features[0],
            screenX: screenX,
            screenY: screenY,
          });
