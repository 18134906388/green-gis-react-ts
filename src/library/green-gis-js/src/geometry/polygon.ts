...
export class Polygon extends Geometry {
  ...
  project(projection: Projection) {
    ...
    this._coordinates = this._lnglats.map((ring: any) =>
      ring.map((point: any) => this._projection.project(point)),
    );
  ...
  draw(
    ...
    symbol: FillSymbol = new SimpleFillSymbol(),
  ) {
    ...
    this._screen = this._coordinates.map(ring => {
      return ring.map((point: any, index) => {
        const screenX = matrix.a * point[0] + matrix.e,
          screenY = matrix.d * point[1] + matrix.f;
  ...
  contain(screenX: number, screenY: number): boolean {
    ...
    return (
      _pointInPolygon([screenX, screenY], first) &&
      others.every(ring => !_pointInPolygon([screenX, screenY], ring))
    );
   ...
   */
  getCenter(
    type: CoordinateType = CoordinateType.Latlng,
    projection: Projection = new WebMercator(),
  ) {
    if (!this._projected) this.project(projection);
    let i,
    ...
    this._coordinates.forEach((ring, index) => {
      if (index == 0) {
        ring.forEach((point, index) => {
          if (index > 0) {
            sum += Math.sqrt(
              Math.pow(point[0] - ring[index - 1][0], 2) +
                Math.pow(point[1] - ring[index - 1][1], 2),
            );
          }
        });
      }
