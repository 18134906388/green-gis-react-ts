  ...
  animate(elapsed, ctx: CanvasRenderingContext2D) {
    ...
    for (let i = 0; i < this.ring; i++) {
      ...
      ctx.arc(
        this._screenX,
        this._screenY,
        ((elapsed / 1000) * this.velocity + (i * this.limit) / this.ring) % this.limit,
        0,
        Math.PI * 2,
        true,
      );
...
export class LineAnimation extends Animation {
  ...
  init(ctx: CanvasRenderingContext2D, projection: Projection = new WebMercator()) {
    ...
    const d = Math.sqrt(
      (this._end[1] - this._start[1]) * (this._end[1] - this._start[1]) +
        (this._end[0] - this._start[0]) * (this._end[0] - this._start[0]),
    );
  ...
  _drawCurvePath(ctx, start, point, end, percent) {
    ...
    for (let t = 0; t <= percent / 100; t += 0.005) {
      const x = this._quadraticBezier(start[0], point[0], end[0], t);
      const y = this._quadraticBezier(start[1], point[1], end[1], t);
      ctx.lineTo(x, y);
    }
    ctx.stroke();
  }
