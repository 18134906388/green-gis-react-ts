...
export class Viewer extends Subject {
  ...
  _onClick(event) {
    ...
    layers
      .filter(layer => layer instanceof FeatureLayer && layer.interactive && !layer.editing)
      .reverse()
      .some((layer: FeatureLayer) =>
        layer.contain(
          event.offsetX,
          event.offsetY,
  ...
  redraw() {
    ...
    this._layers
      ...
      .filter(
        layer =>
          (layer instanceof FeatureLayer && !layer.editing) || !(layer instanceof FeatureLayer),
      )
      .forEach(layer => {
        layer.draw(this._ctx, this._map.projection, this._map.extent, this._map.zoom);
      });
    this._layers
      .filter(layer => layer instanceof FeatureLayer && layer.labeled && !layer.editing)
