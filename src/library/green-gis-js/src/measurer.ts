...
export class Measurer extends Subject {
  ...
  constructor(map: Map) {
    super([
      ...
      'create',
      'delete',
    ]); //when mouseover feature or vertex
    this._map = map;
  ...
  redraw() {
    ...
    this._createLayer &&
      this._createLayer.draw(this._ctx, this._map.projection, this._map.extent, this._map.zoom);
    this._measureLayer &&
      this._measureLayer.draw(this._ctx, this._map.projection, this._map.extent, this._map.zoom);
  ...
  _label(geometry: Geometry) {
    const projection: Projection = this._map.projection;
    geometry.project(projection);
    const ctx: CanvasRenderingContext2D = this._canvas.getContext('2d');
    ctx.save();
    const symbol: SimpleTextSymbol = new SimpleTextSymbol();
    ...
    if (geometry instanceof Polyline) {
      ...
      text =
        length > 1000 ? Math.round((length / 1000) * 10) / 10 + '公里' : Math.round(length) + '米';
      center = geometry.coordinates[geometry.coordinates.length - 1];
    } else if (geometry instanceof Polygon) {
      const area = geometry.getArea(projection);
      text =
        area > 1000 ? Math.round((area / 10000) * 10) / 10 + '公顷' : Math.round(area) + '平方米';
    } else {
      return;
    }
