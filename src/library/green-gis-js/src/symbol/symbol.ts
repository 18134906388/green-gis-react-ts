...
export class PointSymbol extends Symbol {
   ...
   */
  contain(anchorX, anchorY, screenX, screenY): boolean {
    return false;
  }
}
/**
...
export class SimpleTextSymbol extends Symbol {
   ...
   * 边框色
   */
  public strokeStyle = '#ff0000'; //#ffffff
  /**
   * 填充色
   */
  public fillStyle = '#ffffff'; //#ffffff
  /**
   * X偏移
   */
  public offsetX = 0;
   ...
   * 放置位置
   */
  public placement = 'BOTTOM'; //BOTTOM TOP LEFT RIGHT
  /**
   * 自动调整位置
   */
  public auto = false;
   ...
   * 标注点符号的宽度
   */
  public pointSymbolWidth = 0;
  /**
   * 标注点符号的高度
   */
  public pointSymbolHeight = 0;
...
export class GradientPointSymbol extends PointSymbol {
   ...
   * 渐变起始色
   */
  public startColor = '#ff0000'; //#ff0000
  /**
   * 渐变终止色
   */
  public endColor = '#ff0000'; //#ff0000
  /**
   * 绘制点
   * @param {CanvasRenderingContext2D} ctx - 绘图上下文
   * @param {number} screenX - 屏幕坐标X
...
export class SimpleMarkerSymbol extends PointSymbol {
  ...
  contain(anchorX, anchorY, screenX, screenY) {
    return (
      screenX >= anchorX + this.offsetX &&
      screenX <= anchorX + this.offsetX + this.width &&
      screenY >= anchorY + this.offsetY &&
      screenY <= anchorY + this.offsetY + this.height
    );
  }
}
...
export class ArrowSymbol extends Symbol {
  ...
  draw(ctx: CanvasRenderingContext2D, screen: number[][]) {
    ...
    const _getPointAlongLine2 = (k, b, p, d) => {
      const x0 = p[0] + Math.sqrt((d * d) / (k * k + 1)),
        x1 = p[0] - Math.sqrt((d * d) / (k * k + 1));
      return [
        [x0, k * x0 + b],
        [x1, k * x1 + b],
      ];
    ...
    screen.reduce((prev, cur) => {
      if (prev) {
        const length = Math.sqrt(
          (cur[0] - prev[0]) * (cur[0] - prev[0]) + (cur[1] - prev[1]) * (cur[1] - prev[1]),
        );
        if (length >= this.minLength) {
          //中点 即箭头
          const [middleX, middleY] = [(prev[0] + cur[0]) / 2, (prev[1] + cur[1]) / 2];
          //箭尾垂线的垂足
          const [footX, footY] = _getPointAlongLine(
            [middleX, middleY],
            prev,
            Math.cos(this.arrowAngle) * this.arrowLength,
          );
          const k = (cur[1] - prev[1]) / (cur[0] - prev[0]);
          // 1/k 垂线
          const points = _getPointAlongLine2(
            -1 / k,
            footY - (footX * -1) / k,
            [footX, footY],
            Math.sin(this.arrowAngle) * this.arrowLength,
          );
          //两点
...
export class ClusterSymbol extends PointSymbol {
   ...
   * 重写描边样式
   */
  public strokeStyle = '#ffffff'; //#ff0000
  /**
   * 聚合外圈填充样式
   */
  public outerFillStyle = '#ffffff'; //#ff0000
  /**
   * 聚合数量字体颜色
   */
   ...
   * 色带起始色
   */
  public startColor = '#19caad';
  /**
   * 色带终止色
   */
  public endColor = '#f4606c';
  /**
   * 聚合数量文本
   * @remarks
   * 大于99，标记为99+
...
export class VertexSymbol extends PointSymbol {
  ...
  contain(anchorX, anchorY, screenX, screenY) {
    return (
      screenX >= anchorX - this.size / 2 &&
      screenX <= anchorX + this.size / 2 &&
      screenY >= anchorY - this.size / 2 &&
      screenY <= anchorY + this.size / 2
    );
  }
}
